...
export const bitManipulationProblems: ProblemFixture[] = [
  {
    problem: {
      ...
      solution: "XOR all elements together. Since a XOR a = 0 and a XOR 0 = a, all duplicate pairs cancel out, leaving only the single number.",
      timeLimitMs: 5000,
      source: "curated",
      sourceUrl: null,
    },
    testCases: [
      ...
      { input: { nums: [-30000, 30000, -30000] }, expectedOutput: 30000, isVisible: false, orderIndex: 7 },
    ],
    boilerplate: {
      language: "python",
    ...
    hints: [
      { hintText: "XOR has the property that a ^ a = 0 and a ^ 0 = a. XOR all the numbers together and the duplicates will cancel out, leaving the single number.", orderIndex: 0 },
    ],
  },
  {
    problem: {
      ...
      solution: "Use the trick n & (n - 1) to clear the lowest set bit. Count how many times you can do this before n becomes 0. Each iteration removes exactly one set bit.",
      timeLimitMs: 5000,
      source: "curated",
      sourceUrl: null,
    },
    ...
    hints: [
      { hintText: "The operation n & (n - 1) clears the lowest set bit of n. Count how many times you can perform this operation before n becomes 0.", orderIndex: 0 },
    ],
  },
  {
    problem: {
      ...
      solution: "Use the DP relation: the number of 1 bits in i equals the number of 1 bits in i >> 1 (right shift) plus i & 1 (the last bit). This builds the answer iteratively.",
      timeLimitMs: 5000,
      source: "curated",
      sourceUrl: null,
    },
    testCases: [
      ...
      { input: { n: 7 }, expectedOutput: [0, 1, 1, 2, 1, 2, 2, 3], isVisible: false, orderIndex: 5 },
      { input: { n: 8 }, expectedOutput: [0, 1, 1, 2, 1, 2, 2, 3, 1], isVisible: false, orderIndex: 6 },
      { input: { n: 10 }, expectedOutput: [0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2], isVisible: false, orderIndex: 7 },
    ],
    boilerplate: {
      language: "python",
    ...
    hints: [
      { hintText: "Use the relation: countBits(i) = countBits(i >> 1) + (i & 1). The number of set bits in i is the same as i with its last bit removed, plus whether the last bit is set.", orderIndex: 0 },
    ],
  },
  {
    problem: {
      ...
      solution: "Iterate through 32 bits. Shift the result left by 1 and add the least significant bit of n. Then right shift n by 1. Repeat 32 times.",
      timeLimitMs: 5000,
      source: "curated",
      sourceUrl: null,
    },
    ...
    hints: [
      { hintText: "Process one bit at a time. Extract the least significant bit of n, shift your result left to make room, and append the extracted bit. Do this 32 times.", orderIndex: 0 },
    ],
  },
  {
    problem: {
      ...
      solution: "XOR all numbers from 0 to n with all elements in the array. Since XOR cancels out pairs, the remaining value is the missing number. Alternatively, use the sum formula n*(n+1)/2 minus the array sum.",
      timeLimitMs: 5000,
      source: "curated",
      sourceUrl: null,
    },
    testCases: [
      ...
      { input: { nums: [9, 6, 4, 2, 3, 5, 7, 0, 1] }, expectedOutput: 8, isVisible: true, orderIndex: 2 },
      ...
      { input: { nums: [0, 1, 2, 3, 4, 5, 6, 7, 8] }, expectedOutput: 9, isVisible: false, orderIndex: 6 },
    ...
    hints: [
      { hintText: "XOR all indices 0..n with all array elements. Paired values cancel out (a ^ a = 0), leaving only the missing number. Alternatively, compute n*(n+1)/2 - sum(nums).", orderIndex: 0 },
    ],
  },
  {
    problem: {
      ...
      solution: "Use bitwise operations: XOR gives the sum without carry, AND shifted left gives the carry. Repeat until carry is 0. In Python, use a 32-bit mask to handle negative numbers correctly.",
      timeLimitMs: 5000,
      source: "curated",
      sourceUrl: null,
    },
    ...
    hints: [
      { hintText: "XOR (^) computes the sum without considering carry. AND (&) shifted left by 1 computes the carry. Repeat this process until the carry is 0.", orderIndex: 0 },
      { hintText: "In Python, integers have arbitrary precision, so you need a 32-bit mask (0xFFFFFFFF) to simulate 32-bit overflow. After the loop, if the result exceeds 0x7FFFFFFF, convert it back to a negative number.", orderIndex: 1 },
    ],
  },
  {
    problem: {
      ...
      solution: "Extract digits from the end using modulo and division. Build the reversed number digit by digit. Before each multiplication by 10, check for overflow against INT_MAX/10 and INT_MIN/10.",
      timeLimitMs: 5000,
      source: "curated",
      sourceUrl: null,
    },
    ...
    hints: [
      { hintText: "Extract digits from the end using x % 10 and x // 10. Build the reversed number by multiplying by 10 and adding each digit.", orderIndex: 0 },
      { hintText: "Before appending each digit, check if the result would overflow the 32-bit signed integer range. If result > 2^31 / 10 (or the boundary case with remainder), return 0.", orderIndex: 1 },
    ],
  },
];
