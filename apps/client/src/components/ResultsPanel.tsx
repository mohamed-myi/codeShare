  ...
  if (lastError) {
    return (
      <div className="p-4" data-testid="results-panel">
        <div className="border-l border-[var(--color-error)] pl-4 text-sm text-[var(--color-error-text)]">
          {lastError}
        </div>
      </div>
    );
...
function RunResultView({ result }: { result: RunResult }) {
  ...
  return (
    <div className="overflow-y-auto px-4 py-5" data-testid="results-panel">
      <div className="mb-2 flex items-center gap-2">
        ...
        <span
          className={`text-sm ${allPassed ? "text-[var(--color-success-text)]" : "text-[var(--color-error-text)]"}`}
        >
      ...
      {result.userStdout && (
        <div className="mt-3 border-t border-[var(--color-border-subtle)] pt-2">
          <h4 className="mb-2 text-xs uppercase tracking-[0.08em] text-[var(--color-text-tertiary)]">
            Console Output
          </h4>
...
function CaseResultView({ caseResult }: { caseResult: CaseResult }) {
  return (
    <div className="border-l border-[var(--color-border-subtle)] pl-4 text-xs">
...
function SubmitResultView({ result }: { result: SubmitResult }) {
  ...
  return (
    <div className="overflow-y-auto px-4 py-5" data-testid="results-panel">
      ...
      {result.firstFailure && (
        <div className="border-l border-[var(--color-error)] pl-4 text-xs">
          <div className="mb-1 text-[var(--color-error-text)]">
